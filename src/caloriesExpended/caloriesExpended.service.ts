...
export class CaloriesExpendedService{
    constructor(
        @InjectModel(CaloriesExpended.name) private readonly caloriesExpendedModel: PaginateModel<CaloriesExpended>){}
    
    private readonly logger = new Logger(CaloriesExpendedService.name);

    async create(createCaloriesExpendedDto: CreateCaloriesExpendedDto, user : User): Promise <CaloriesExpended>{
        try{
